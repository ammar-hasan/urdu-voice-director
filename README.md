...
Current release: **0.5.0 structurally validated beta**.
...
Pronunciation uses an ambiguity-first provider copy while captions stay canonical. Vowel marks resolve established ambiguous readings; they are not treated as fixes for consonant substitutions such as `ڑ`→`ر` or `ٹ`→`ت`. Current Eleven v3 adapters may test selective inline IPA or pronunciation dictionaries, always against the exact voice and a no-control baseline. Inline IPA replaces the canonical token only in the provider pronunciation copy; the payload must not send both spellings at the same locus.

TTS runs record the relevant routed references before adaptation. Continuous same-speaker thoughts are not split merely to manufacture pauses, and punctuation, tags, segmentation, renderer gaps, and padding are tested as separate controls.
...
Audit a real JSON adapter or trusted default-exported `.mjs` configuration separately:
...
- a production-adapter audit for caption drift, duplicated IPA, missing runtime/reference provenance, ASR-overclaiming, tag density, and stacked pause controls;
